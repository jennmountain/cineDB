...
const QUICK_QUERIES = [
  {
    label: '🎬 Top 10 Movies',
    sql: `select title, genre, release_year, avg_rating, imdb_rating as rt_score
from media
where type = 'movie' and avg_rating > 0
order by avg_rating desc
limit 10;`,
  },
  {
    label: '📺 Top 10 TV Shows',
    sql: `select title, genre, release_year, avg_rating, seasons, episodes, imdb_rating as rt_score
from media
where type = 'tv' and avg_rating > 0
order by avg_rating desc
limit 10;`,
  },
  {
    label: '🍅 RT Score Above 90%',
    sql: `select title, type, genre, release_year, imdb_rating as rt_score
from media
where imdb_rating > 90
order by imdb_rating desc, title;`,
  },
  {
    label: '🎭 Most Credited Actors',
    sql: `select p.name, count(*) as appearances
...
limit 15;`,
  ...
  {
    label: '🎥 Directors by Rating',
    sql: `select p.name, count(m.id) as films_directed, round(avg(m.avg_rating), 1) as avg_rating
from media_credits mc
join people p on p.id = mc.person_id
join media m on m.id = mc.media_id
where mc.role = 'director' and m.avg_rating > 0
group by p.id, p.name
order by avg_rating desc
limit 15;`,
  },
  {
    ...
    sql: `select m.title, m.type, m.genre, m.avg_rating, p.username as recommended_by, r.created_at
